...
| **Encrypted backup** | `cws backup setup /target && cws backup run` — restic under the hood: client-side encrypted, incremental, any target restic speaks (disk, SFTP, S3, your rclone remotes) |
| **Per-device sessions** | `cws session add alice` — a second (third…) concurrent desktop for one member, own hostname + Access policy + Claude sign-in; new client = new session |
